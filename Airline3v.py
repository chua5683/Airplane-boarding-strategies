...
#random.shuffle(availble_seats)
def visualboard(seatList,step, speed,personal_space,ctpp,space_needed,speedList,bagList):
    ...
    tk.title("Airplane Queue")
    canvas.pack()
    canvas.create_line(0,100,600,100)
    canvas.create_line(0,85,600,85)
    peopleList=[]
    for i in range(len(seatList)):
         #this is random seating
        peopleList.append(personc(0-i, speedList[i], seatList[i],bagList[i]))
    t=0
    while sum([not(i.seated) for i in peopleList])>0: #we board until everyone is seated
        ...
        tk.update()
        time.sleep(speed)
    canvas.create_text(500,300, text="Time= %f"%t)
    return t
    canvas.mainloop()
...
class personc:
    def __init__(self, xpos,speed, seat, bagspeed):#first initallize all attributes of the person        
        ...
        self.shape=canvas.create_oval(xpos*pixels,100,xpos*pixels+15,85,fill="green")
        self.color="green"
            ...
            if self.loading: #if you are in a loading stage :
                ...
                if self.wait>=self.bagspeed: #when you finish waiting :
                    if self.climbing==False:
                        ...
                        if ppr > 0:
                            ...
                            self.color="blue"
                            canvas.itemconfig(self.shape,fill=self.color)
                            canvas.coords(self.shape,(self.seat[0])*pixels,100,(self.seat[0])*pixels+15,85)
                            if self.seat[1]<split:
                                j=0
                            else:
                                j=1
                            canvas.move(self.shape, 0,(self.seat[1]-len(matrix)/2+j)*15)
                    else:
                        self.climbtime+=step
                ...
                if self.seat[0]-space_needed-self.xpos<.001:
                    if not(someone_there(self.seat[0],peopleList[:index],personal_space)):  
                        ...
                        self.color="red"
                        canvas.itemconfig(self.shape,fill=self.color)
                        canvas.coords(self.shape,(self.seat[0]-space_needed)*pixels,100,(self.seat[0]-space_needed)*pixels+15,85)
                        self.xpos=canvas.coords(self.shape)[0]/pixels
                else:
                    if not(someone_there(newpos,peopleList[:index],personal_space)): #if there is no one in front of you :
                        ...
                        canvas.move(self.shape, self.speed*step*pixels,0)
#visualboard(availble_seats,.2,.001,1.2,2,1)
